...
A high-level overview of Iris testbed architecture is depicted in Fig. 1. The *functional elements* layer, at the bottom, represent the tangible resources including servers, switches, USRPs, and so forth, at the Iris testbed. The *virtualisation layers* (nfv and virtualisation layers) in the middle are supported by virtualisation technologies including OpenStack to support cloud computing, OpenFlow to slice the network, and HyDRA to virtualise the radio. The *experiments layer* (experiments and virtualised experiments) at the top, supported by the Open Source Network Function Virtualization (NFV) Management and Orchestration (MANO) (OSM) software stack in the virtualisation layers, interacts with the physical and virtualisation layers to instantiate experimental vertical instances (EVIs). These technologies interact with the FUTEBOL EU project Container Orchestration and Provisioning Architecture (COPA) virtual machine instances  to support monitoring NFV functions in wireless, packet and optical networks, Ubuntu Containers (LXC), and GNU Radio images, exposing the functionality of physical resources to applications and across different 5G verticals.
