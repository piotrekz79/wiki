...
## Experimentation
Experimentation activities is supported in the eHealth5G EVI through the 5GinFIRE portal. Using the portal the experimenters are able to:
* create the experiment using eHealth5G facility
* deploy virtual functions (VxF) at the eHealth5G Edge and Core Cloud, which will process data ingest from eHealth devices,
* access to deployed virtual machines (VxF) at the eHealth5G Edge or Core Cloud infrastructure.
...
* ask for sending data gathered on eHealth devices to the eHealth Cloud,
* request high-speed network connection (up to 1Gbps) to other 5GinFIRE facilities (or any site directly connected to research&education GÉANT network) allowing for combining resources and functionalities of different facilities,
